...
public class CardsMatchingFrequencySteps {
  ...
  public void
  the_system_has_cards_in_various_categories_with_last_revision_dates() {
    cardService.createCard(new CardUserData("What is the capital of France?",
                                            "Paris", "geography"));
    cardService.createCard(new CardUserData("What is the capital of Germany?",
                                            "Berlin", "geography"));
    cardService.createCard(
        new CardUserData("What is the capital of Italy?", "Rome", "geography"));
    cardService.createCard(new CardUserData("What is the capital of Spain?",
                                            "Madrid", "geography"));
    cardService.createCard(new CardUserData("What is the capital of Portugal?",
                                            "Lisbon", "geography"));
    cardService.createCard(new CardUserData("What is the capital of England?",
                                            "London", "geography"));
    cardService.createCard(new CardUserData("What is the capital of the USA?",
                                            "Washington", "geography"));
    cardService.createCard(new CardUserData("What is the capital of Canada?",
                                            "Ottawa", "geography"));
    ...
    for (int i = 1; i < cards.size(); i++) {
      ...
      Category nextCategory =
          categories[Math.min(i + 1, categories.length - 1)];
      while (Objects.requireNonNull(card).getCategory() != nextCategory) {
        cardService.processCardAnswer(card.getId(), true);
        Card finalCard = card;
        card = cardService.fetchAllCards(Optional.empty())
                   .stream()
                   .filter(c -> c.getId().equals(finalCard.getId()))
                   .findFirst()
                   .orElse(null);
      }
    }
  ...
  @Then(
      "only cards eligible for review based on their category frequency are shown")
  public void
  only_cards_eligible_for_review_based_on_their_category_frequency_are_shown() {
    for (Card card : cardsForQuiz) {
      CardRevision lastRevision =
          revisionService.findLatestRevisionByCardId(card.getId());
      LocalDate lastRevisionDate;
      if (lastRevision != null && lastRevision.getRevisionDate() != null) {
        lastRevisionDate = LocalDate.parse(lastRevision.getRevisionDate(),
      ...
      long daysSinceLastRevision =
          ChronoUnit.DAYS.between(lastRevisionDate, quizDate);
      int expectedDaysUntilNextRevision =
          categoryService.getDaysUntilNextRevision(card.getCategory());
      boolean isEligibleForReview =
          daysSinceLastRevision >= expectedDaysUntilNextRevision;

      assertTrue(
          isEligibleForReview,
          String.format(
              "Card with ID %s should be eligible for review based on its category frequency.",
              card.getId()));
    }

    if (quizDate.isEqual(LocalDate.now().plusDays(1))) {
      boolean parisIsEligible = cardsForQuiz.stream().anyMatch(
          card
          -> "Paris".equals(card.getAnswer()) &&
                 card.getCategory() == Category.FIRST);
      assertTrue(parisIsEligible,
                 "Paris should be eligible for review the day after creation.");
    }
  }
}
