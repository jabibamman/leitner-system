...
public class UserServiceTests {
  ...
  public void login_returns_the_user_when_found() {
    ...
    Optional<UserEntity> result = Optional.ofNullable(userService.login("username", "password"));
  ...
  public void test_login_with_invalid_credentials() {
    ...
    when(userRepository.login("invalid_username", "invalid_password")).thenReturn(Optional.empty());
  ...
  public void register_save_the_user_with_the_good_credentials() {
    ...
    when(userRepository.login(user.getUsername(), user.getPassword())).thenReturn(Optional.of(user));
    Optional<UserEntity> result = Optional.ofNullable(userService.login(user.getUsername(), user.getPassword()));
  ...
  @Test
  public void register_throws_UserAlreadyExistsException_when_user_already_exists() {
        UserEntity user = new UserEntity("username", "password");
        UserRepositoryPort userRepository = mock(UserRepositoryPort.class);
        UserService userService = new UserService(userRepository);
